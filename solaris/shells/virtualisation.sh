...
search_file() {
    ...
    local count=$(find "$directory" -maxdepth 1 -type f -name "*$search_term*" 2>/dev/null | wc -l)
...
search_word() {
    ...
    local count=$(grep -o "$search_word" "$file" | wc -l)
...
search_file_and_word() {
    ...
    local files=$(find "$directory" -maxdepth 1 -type f -name "*$file_search_term*" 2>/dev/null)
    if [ -z "$files" ]; then
        echo "No files found."
        return
    fi
    ...
    for file in $files; do
        ...
        local count=$(grep -o "$word_search_term" "$file" | wc -l)
